// src/hooks/useSkinScrapingForm.ts
...
import { SKIN_SCRAPING_TABS, getLocalDateString } from "@/constants/skinScraping";
import type { LabExam } from "@/types/labExam";
import type {
  SkinScrapingFormData,
  SkinScrapingTabId,
  SkinScrapingPaymentData,
} from "@/types/labExam/skinScraping";

export function useSkinScrapingForm() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const [activeTab, setActiveTab] = useState<SkinScrapingTabId>("patient");
  ...
  const form = useForm<SkinScrapingFormData>({
    defaultValues: {
      patientName: "",
      species: "canino",
      breed: "",
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: SkinScrapingFormData) =>
      createLabExam({
        ...data,
        examType: "skin_scraping",
      } as any),
    onSuccess: (response) => {
  ...
  // Confirmar pago
  const handlePaymentConfirm = (paymentData: SkinScrapingPaymentData) => {
    ...
    const finalData: SkinScrapingFormData = {
      ...getValues(),
  ...
  // Cambiar tab
  const handleTabChange = (tab: SkinScrapingTabId) => {
  ...
  // Submit
  const onSubmit = (data: SkinScrapingFormData) => {
    ...
    if (!data.type) {
      toast.error("Selecciona el tipo de raspado");
      setActiveTab("exam");
      return;
    }
  ...
  const currentTabIndex = SKIN_SCRAPING_TABS.findIndex((t) => t.id === activeTab);
