...
import { Check, Loader2, Syringe, Trash2, Pencil, FlaskConical, Clock, ChevronRight } from "lucide-react";
...
export default function VaccinationView() {
  ...
  const [vaccinationToView, setVaccinationToView] = useState<Vaccination | null>(null);
  ...
  const handleOpenDetail = (vaccination: Vaccination, e: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const getVaccineColor = (type: string) => {
    ...
    if (t.includes("rabia")) return "text-red-500 bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800";
    if (t.includes("sextuple") || t.includes("múltiple")) return "text-blue-500 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800";
    if (t.includes("parvo")) return "text-purple-500 bg-purple-50 dark:bg-purple-950/30 border-purple-200 dark:border-purple-800";
    if (t.includes("tos") || t.includes("kennel")) return "text-amber-500 bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800";
    return "text-biovet-500 bg-biovet-50 dark:bg-biovet-950/30 border-biovet-200 dark:border-biovet-800";
  };

  const sortedVaccinations = [...vaccinations].sort((a, b) => new Date(b.vaccinationDate).getTime() - new Date(a.vaccinationDate).getTime());

  if (isLoading) return <div className="h-64 flex items-center justify-center"><Loader2 className="w-8 h-8 text-biovet-500 animate-spin" /></div>;
  ...
  return (
    ...
    >
      {sortedVaccinations.map((vac) => {
        const isPending = vac.nextVaccinationDate && new Date(vac.nextVaccinationDate) > new Date();
        const colorClasses = getVaccineColor(vac.vaccineType);

        return (
          <div key={vac._id} className="relative flex gap-6 md:gap-8 group animate-fade-in">
            {/* Icono con color dinámico */}
            <div className={`relative z-10 shrink-0 w-5 h-5 md:w-6 md:h-6 rounded-lg border flex items-center justify-center shadow-sm transition-transform group-hover:scale-110 ${colorClasses}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 text-xs text-slate-500">
                    ...
                    {vac.laboratory && <span className="flex items-center gap-1 opacity-70"><FlaskConical size={14}/> {vac.laboratory}</span>}
                  </div>
                </div>

                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditItem(vac); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-biovet-500 transition-colors">
                    <Pencil size={18} />
                  </button>
                  <button onClick={() => setDeleteId(vac._id!)} className="p-2 text-slate-400 hover:text-danger-500 transition-colors">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
              ...
              {vac.nextVaccinationDate && (
                <div className="mt-3 flex items-center gap-2 text-[13px] text-slate-500 font-medium italic">
                  {isPending ? <Clock size={14} className="text-warning-500" /> : <Check size={14} className="text-success-500" />}
                  Refuerzo: <span className={`font-bold ${isPending ? 'text-warning-600' : 'text-success-600'}`}>
