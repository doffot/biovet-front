...
export default function CreateLabExamView() {
  ...
  const calculatedValues = useMemo(() => {
    ...
    if (!differentialCount) return calculated;

    Object.keys(differentialCount).forEach((key) => {
      const cellKey = key as keyof NonNullable<LabExamFormData["differentialCount"]>;
      const percentage =
  ...
  const handleUndo = () => {
    ...
    const currentValue = differentialCount?.[field] ?? 0;
    if (currentValue > 0) {
      setDifferentialCount((prev) => {
        if (!prev) return prev;
        return {
