    ...
    <>
      ...
      <div className="hidden lg:block h-full">
        <div className="grid grid-cols-12 gap-6 h-full">
          <div className="col-span-12 xl:col-span-5 h-150">
            <PatientMedicalLog patientId={patient._id!} />
          </div>

          <div className="col-span-12 xl:col-span-7 space-y-6 h-150 overflow-y-auto custom-scrollbar pr-2">
            {/* CARD DESPARASITACIÓN */}
            <HealthCard title="Desparasitación Interna" icon={<ShieldCheck className="text-biovet-500" size={20} />} status={dwHealth.status} colorClass={dwHealth.colorClass}>
              <HealthRow 
                name="Control de Parásitos Internos" 
                isMissing={dwHealth.status === "Sin registros"} 
                alert={dwHealth.alert} 
                nextDate={dwHealth.nextDate}
                product={dwHealth.productName}
            ...
            {/* CARD VACUNAS */}
            <HealthCard title="Plan de Vacunación" icon={<Syringe className="text-biovet-500" size={20} />} status={vaxHealth.status} colorClass={vaxHealth.colorClass}>
              {[...vaxHealth.applied, ...vaxHealth.missing].map((name) => (
                <HealthRow key={name} name={name} isMissing={vaxHealth.missing.includes(name)} alert={vaxHealth.alerts.find(a => a.name === name)} showDetails />
              ))}
            </HealthCard>

            {/* Observaciones */}
            {patient.observations && (
              <div className="bg-danger-50 dark:bg-danger-900/10 p-5 rounded-2xl border border-danger-100 dark:border-danger-900/30 flex gap-4 items-start">
                <div className="p-2 bg-white dark:bg-danger-900/20 rounded-lg text-danger-500"><Info size={20} /></div>
                <div>
                  <p className="text-[10px] font-bold text-danger-400 uppercase mb-1">Nota Clínica</p>
