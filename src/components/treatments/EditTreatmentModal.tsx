...
export default function EditTreatmentModal({ isOpen, onClose, treatment }: Props) {
  ...
  useEffect(() => {
    if (isOpen && treatment) {
      ...
      setValue("treatmentTypeOther", treatment.treatmentTypeOther ?? undefined);
      setValue("route",              treatment.route);
      setValue("routeOther",         treatment.routeOther ?? undefined);
      ...
      setValue("observations",       treatment.observations ?? undefined);

     
      setValue("startDate", treatment.startDate.split("T")[0]);
      setValue(
        "endDate",
        treatment.endDate ? treatment.endDate.split("T")[0] : undefined
      );
    }
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["treatments", treatment.patientId],
      });
      onClose();
    },
    onError: (error: Error) => {
  ...
  const onSubmit = (data: TreatmentFormData) => {
    const payload: TreatmentFormData = {
      ...data,
      // ✅ limpiar campos condicionales
      treatmentTypeOther:
        data.treatmentType === "Otro"
          ? data.treatmentTypeOther?.trim() || undefined
          : undefined,
      routeOther:
        data.route === "Otro"
          ? data.routeOther?.trim() || undefined
          : undefined,
