  ...
  return (
    <div>
      <button
        ...
        className={`w-full group relative flex items-center justify-between px-2.5 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium cursor-pointer ${hasActiveChild ? 'bg-white/5 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white'} ${collapsed ? 'justify-center px-2' : ''}`}
      >
        <div className="flex items-center gap-2.5 overflow-hidden">
          {/* Icono cambia al VERDE LIMA del logo si está activo */}
          <item.icon className={`shrink-0 transition-colors ${collapsed ? 'w-5 h-5' : 'w-4 h-4'} ${hasActiveChild ? 'text-success-500' : ''}`} />
        ...
        <div className="ml-3 pl-3 border-l border-white/10 space-y-0.5 py-0.5">
          {item.submenu?.map((subItem) => (
            <NavLink 
              ...
              className={({ isActive }) => `flex items-center gap-2 px-2 py-1 rounded-md text-[11px] transition-all duration-200 ${isActive ? "text-success-400 font-medium bg-success-500/10" : "text-slate-500 hover:text-white hover:bg-white/5"}`}
            >
              {/* El punto indicador ahora es Verde Success como la huella */}
              <span className={`w-1 h-1 rounded-full ${location.pathname.startsWith(subItem.to) ? 'bg-success-500 shadow-[0_0_8px_rgba(132,204,22,0.6)]' : 'bg-slate-600'}`} />
...
export const Sidebar = () => {
  ...
  const SidebarContent = ({ collapsed, isMobile = false }: { collapsed: boolean; isMobile?: boolean }) => (
    <div className="flex flex-col h-full">
      {/* HEADER LOGO */}
      <div className={`flex items-center h-16 shrink-0 transition-all duration-300 ${isMobile ? 'justify-between px-4' : 'justify-center'}`}>
        <NavLink to="/" className="flex items-center gap-2.5 overflow-hidden px-2 group">
          ...
          {!collapsed && (
            <div className="flex flex-col animate-fade-in overflow-hidden">
              <span className="text-white font-heading font-bold text-[13px] leading-tight truncate uppercase tracking-tight">
                {clinic?.name || "BioVet Track"}
              </span>
            </div>
          )}
        </NavLink>
      ...
      {!isMobile && (
        <div className="relative flex items-center justify-center h-4 shrink-0 px-2">
          ...
          <button onClick={toggleSidebar} className="hidden lg:flex absolute -right-2.5 w-5 h-5 rounded-full bg-biovet-500 border border-white/10 items-center justify-center text-white hover:bg-success-500 hover:scale-110 transition-all duration-200 shadow-xl z-50 cursor-pointer">
      ...
      <nav className="flex-1 py-4 px-3 space-y-6 overflow-y-auto scrollbar-none">
        {menuItems.map((section, idx) => (
          <div key={idx} className="space-y-2">
            {!collapsed && (
              <h3 className="px-3 mb-1 text-[9px] font-black text-slate-600 uppercase tracking-[0.2em]">{section.groupLabel}</h3>
            )}
      ...
      <div className="shrink-0 p-3 space-y-2 border-t border-white/5 bg-black/20">
        ...
        <button
          ...
          className={`w-full group relative flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-all duration-200 text-xs font-medium cursor-pointer text-slate-500 hover:bg-danger-500/10 hover:text-danger-400 ${collapsed ? 'justify-center' : ''}`}
        >
