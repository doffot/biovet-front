// src/components/labexam/skinscraping/SkinScrapingFooter.tsx
...
interface SkinScrapingFooterProps {
...
export function SkinScrapingFooter({
  activeTab,
  currentTabIndex,
  isPatientSelected,
  isPending,
    ...
    <footer className="shrink-0 fixed bottom-0 left-0 right-0 sm:relative bg-white dark:bg-dark-200 border-t border-surface-200 dark:border-dark-100 px-4 sm:px-6 py-3 sm:py-4 mb-16 sm:mb-0 z-10">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          {SKIN_SCRAPING_TABS.map((tab, index) => {
          ...
          <span className="text-xs text-slate-400 ml-2">
            Paso {currentTabIndex + 1} de {SKIN_SCRAPING_TABS.length}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto">
          <button
            ...
            onClick={() => {
              if (currentTabIndex > 0)
                onTabChange(SKIN_SCRAPING_TABS[currentTabIndex - 1].id);
            }}
          ...
          {activeTab === "results" ? (
            <button
              ...
              className="px-4 sm:px-8 py-2.5 flex-1 md:flex-none flex items-center justify-center gap-1.5 sm:gap-2 text-sm rounded-lg font-semibold transition-all bg-amber-600 hover:bg-amber-700 text-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isPending ? (
            ...
            <button
              ...
              onClick={() => {
                ...
                if (currentTabIndex < SKIN_SCRAPING_TABS.length - 1) {
                  onTabChange(SKIN_SCRAPING_TABS[currentTabIndex + 1].id);
                }
              }}
              disabled={activeTab === "patient" && !isPatientSelected}
              className="px-4 sm:px-8 py-2.5 flex-1 md:flex-none flex items-center justify-center gap-1 sm:gap-2 text-sm rounded-lg font-semibold transition-all bg-amber-600 hover:bg-amber-700 text-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <span>Siguiente</span>
