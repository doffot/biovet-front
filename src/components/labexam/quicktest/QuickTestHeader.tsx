// src/components/labexam/quicktest/QuickTestHeader.tsx
...
interface QuickTestHeaderProps {
  patientName: string;
  species: string;
  breed?: string;
...
export function QuickTestHeader({
  patientName,
  species,
  breed,
  isPatientSelected,
  ...
  onClearPatient,
  onTabChange,
}: QuickTestHeaderProps) {
  return (
    <header className="shrink-0 bg-linear-to-r from-cyan-600 to-cyan-700 text-white px-4 sm:px-6 py-4 sm:py-5">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 sm:gap-3">
                <Beaker className="w-5 h-5 hidden sm:block" />
        ...
        <nav className="flex gap-1 mt-4 sm:mt-5 bg-white/10 p-1 rounded-xl">
          {QUICK_TEST_TABS.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            const tabIndex = QUICK_TEST_TABS.findIndex((t) => t.id === tab.id);
