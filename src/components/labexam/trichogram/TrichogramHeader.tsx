// src/components/labexam/trichogram/TrichogramHeader.tsx
import { X, TestTube, Check } from "lucide-react";
import { TRICHOGRAM_TABS } from "@/constants/trichogram";
import type { TrichogramTabId } from "@/types/labExam/trichogram";

interface TrichogramHeaderProps {
  patientName: string;
  species: string;
  breed?: string;
...
export function TrichogramHeader({
  patientName,
  species,
  breed,
  isPatientSelected,
  ...
  onClearPatient,
  onTabChange,
}: TrichogramHeaderProps) {
  return (
    <header className="shrink-0 bg-linear-to-r from-teal-600 to-teal-700 text-white px-4 sm:px-6 py-4 sm:py-5">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 sm:gap-3">
                <TestTube className="w-5 h-5 hidden sm:block" />
        ...
        <nav className="flex gap-1 mt-4 sm:mt-5 bg-white/10 p-1 rounded-xl">
          {TRICHOGRAM_TABS.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            const tabIndex = TRICHOGRAM_TABS.findIndex((t) => t.id === tab.id);
